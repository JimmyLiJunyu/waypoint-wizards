...
function DayCard({ day, date, attractions } : {
    day: number;
    date: Date;
    ...
    return (
        <div className='bg-white border rounded-xl p-4 mb-3'>
            ...
            <div
                ref={setNodeRef}
                className={`min-h-20 rounded-lg border-2 border-dashed p-2 flex flex-col gap-2 transition-colors ${isOver ? 'border-red-400 bg-red-50' : 'border-gray-200'}`}>
                        ...
                        strategy={verticalListSortingStrategy}>
                            {attractions.length === 0 ? (
                                ...
                                attractions.map((attraction) => (
                                    <SortableAttractionItem key={attraction.placeId} attraction={attraction} />
                                ))
                            )}
                    </SortableContext>
            </div>
